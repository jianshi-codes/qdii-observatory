...
普通用户可直接打开 <http://127.0.0.1:5173/ops>，在“数据准备向导”点击“同步日常数据”。它会同步近 10 天净值、场内价格、当天直销/代销限额和参考汇率；“仅同步今日限额”用于窄范围重试。任务执行期间页面会继续轮询 ingestion run，刷新页面不会抹掉已经写入数据库的进度。若已有数据操作正在运行，新的操作返回 409，不会并发重复抓取。
