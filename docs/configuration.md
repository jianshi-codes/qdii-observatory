...
| `QDII_EXTERNAL_DATABASE_URL` | 未设置 | 仅供 `make docker-up-external` 使用的既有外部 PostgreSQL database URL |
...
默认 Docker 模式固定使用内置 PostgreSQL。外部模式必须显式执行 `make docker-up-external`；只设置 URL 后运行 `make docker-up` 不会切换数据库。外部 database 必须预先存在且建议专用，详见 [external-postgresql.md](external-postgresql.md)。
