...
“我的持仓”页的“刷新最新净值”按钮复用同一条 `sync-daily` 队列，只同步当前持仓涉及的基金。任务会更新近 10 天净值、场内价格、申购限额与汇率，结束后自动刷新持仓估值；如果存在定投计划，也会重新加载计划金额、费率和到账金额。计划本身不是成交凭证，系统不会据此自动增加份额，实际成交仍需重新导入平台快照。
