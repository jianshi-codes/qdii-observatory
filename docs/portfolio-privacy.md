...
开启后可使用 ignored 的 `.data/private/portfolio.json`，也可在页面下载 XLSX 模板并选择文件。页面先把文件发送到本机 API 做内存预览校验，只有用户点击“确认导入”才写入本地数据库；原始 XLSX 不由应用持久化。项目不把文件上传到第三方，也不连接真实账户。示例使用虚构平台和 synthetic 金额。
